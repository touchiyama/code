...
@click.command(
    help='select gmm center with trained model',
...
def main(seqpath, modelpath, cuda_id, use_cuda, save_dir, fwd, rev):
    ...
    model = CNN_PHMM_VAE(target_len, embed_size=10)
    ...
    with open(save_dir/"gmm_seq.csv","w") as f:
        ...
        for i,(seq, point) in enumerate(zip(sequences, points)):
            logger.info(f"{seq},({point})")
            f.write(f"{i},{seq},{point}\n")
