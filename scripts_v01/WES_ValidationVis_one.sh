...
qsub -V -q ${queue} -o $log_file -e $log_file -j y -N ${prefix} -pe smp 32 \
  python3 ${script_dir}/02_plot_RepRob.py -i ${map_summary} -l ${legend[@]} -y 1 -x1 ${x1[@]} -x2 ${x2[@]} -o ${json_dir} -n ${file_name}

qsub -V -q ${queue} -o $log_file -e $log_file -j y -N ${prefix} -pe smp 32 \
  python3 ${script_dir}/02_plot_RepRob.py -i ${tmb_sumary} -l ${legend[@]} -y 2 -x1 ${x1[@]} -x2 ${x2[@]} -o ${json_dir} -n ${file_name}

qsub -V -q ${queue} -o $log_file -e $log_file -j y -N ${prefix} -pe smp 32 \
  python3 ${script_dir}/02_plot_RepRob.py -i ${msi_sumary[@]} -l ${legend[@]} -y 3 -x1 ${x1[@]} -x2 ${x2[@]} -o ${json_dir} -n ${file_name}

qsub -V -q ${queue} -o $log_file -e $log_file -j y -N ${prefix} -pe smp 32 \
  python3 ${script_dir}/02_plot_RepRob.py -i ${snv_summary} -l ${legend[@]} -y 4 -x1 ${x1[@]} -x2 ${x2[@]} -o ${json_dir} -n ${file_name}

qsub -V -q ${queue} -o $log_file -e $log_file -j y -N ${prefix} -pe smp 32 \
  python3 ${script_dir}/02_plot_RepRob.py -i ${detect_summary} -l ${legend[@]} -y 5 -x1 ${x1[@]} -x2 ${x2[@]} -o ${json_dir} -n ${file_name}

qsub -V -q ${queue} -o $log_file -e $log_file -j y -N ${prefix} -pe smp 32 \
  python3 ${script_dir}/02_plot_RepRob.py -i ${cnv_summary} -l ${legend[@]} -y 6 -x1 ${x1[@]} -x2 ${x2[@]} -o ${json_dir} -n ${file_name}

qsub -V -q ${queue} -o $log_file -e $log_file -j y -N ${prefix} -pe smp 32 \
  python3 ${script_dir}/02_plot_RepRob.py -i ${cnvCorr_summary} -l ${legend[@]} -y 7 -x1 ${x1[@]} -x2 ${x2[@]} -o ${json_dir} -n ${file_name}
