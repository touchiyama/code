    ...
    def data_processing(self):
        ...
        for j in range(len(self.legend)):
            ...
            for k, (tumor_data, normal_data) in enumerate(zip(self.data[0], self.data[1])):
                ...
                #===================   tumor data   ===================#
                y, x, color = extract_data(df, index, self.legend[j], tumor_data,
                                           normal_value, 'tumor', y, x, color)

                #===================  normal data   ===================#
                y, x, color = extract_data(df, index, self.legend[j], normal_data,
                                           tumor_value, 'normal', y, x, color)
...
def extract_data(df, index, legend, data, data_value, sample, y, x, color):
    """目的データを抽出する関数
