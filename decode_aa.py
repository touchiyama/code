...
from raptgen.models_aa import CNN_PHMM_VAE
from raptgen.data_aa import ProfileHMMSampler

dir_path = os.path.dirname(os.path.realpath(__file__))
default_path = str(Path(f"{dir_path}/../out/decode").resolve())

@click.command(
    help='achieve sequence vector in embedded space.',
...
@click.option(
    "--embed-dim",
    help="the embedding dimension of raptgen model",
    type=int,
    default=2
)
@click.option(
    "--eval-max",
    help="the maximum number of sequence to evaluate most probable sequence",
    type=int,
    default=256
)
...
def main(pos_path, model_path, target_len, cuda_id, use_cuda, save_dir, embed_dim, eval_max):
    ...
    device = torch.device(
        f"cuda:{cuda_id}" if (use_cuda and torch.cuda.is_available()) else "cpu"
    )
    ...
    for i, (a, e_m) in enumerate(zip(transition, emission)):
        ...
        products = product(*[list('ALIVMCSNTQDEGPFYWHRK') for _ in range(seq_pattern.count("*"))])
        ...
        if len(rets) > eval_max:
            rets = [rets[idx] for idx in np.argsort(np.random.randn(len(rets)))[:eval_max]]
        
        with Pool() as p:
            probas = p.map(sampler.calc_seq_proba, rets)

        most_probable_seq, min_value = sorted(list(zip(rets, probas)), key=lambda x: x[1])[0]
